  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-xl text-sm font-semibold ring-offset-background transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 cursor-pointer font-display",
  {
    variants: {
      variant: {
        ...
        outline:
          "border border-white/15 bg-transparent text-white/70 hover:bg-white/5 hover:text-white hover:border-white/25",
        secondary:
        ...
        "brand-outline":
          "border-2 border-brand text-brand hover:bg-brand hover:text-white transition-colors",
        royal:
          "text-white hover:scale-[1.02] active:scale-[0.98] shadow-lg",
      },
      size: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, style, ...props }, ref) => {
    ...
    const gradientStyle = variant === "brand"
      ? {
