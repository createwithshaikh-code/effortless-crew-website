...
const floatingBadges = [
  ...
  },
  {
    ...
    pos: "top-36 -right-4 lg:-right-14",
  },
  {
    icon: Sparkles,
    text: "100M+ Views",
    ...
    pos: "bottom-24 -left-4 lg:-left-16",
  },
];
...
export default function Hero() {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden bg-dark-300">
      ...
      {/* Subtle static bg orbs — lightweight, no animation */}
      <div
        className="absolute top-[-15%] right-[-5%] w-[480px] h-[480px] rounded-full pointer-events-none"
        style={{
          background:
            "radial-gradient(circle, rgba(192,38,211,0.12) 0%, transparent 70%)",
        }}
      />
      <div
      ...
      <div className="relative z-10 w-full container mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-0">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-4 lg:min-h-screen">
          ...
          {/* ── LEFT: Text content ── */}
          <div className="relative flex-1 flex flex-col justify-center lg:py-24 lg:pr-8">

            {/* Floating stat badges — desktop only */}
            {floatingBadges.map((badge, i) => (
              <motion.div
                key={badge.text}
                className={`absolute hidden lg:flex items-center gap-2 px-3 py-2 rounded-xl glass border ${badge.border} ${badge.pos}`}
          ...
          {/* ── RIGHT: Solar orbit system — desktop only ── */}
          <motion.div
